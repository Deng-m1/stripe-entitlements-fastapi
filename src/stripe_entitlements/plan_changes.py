...
from .transitions import BillingInterval, TransitionDecision, decide_transition
...
class PlanChangeCoordinator:
    ...
    def preview(self, account: Mapping[str, Any], plan: str, interval: str) -> TransitionDecision:
        ...
        return decide_transition(current, current_interval, target, target_interval)
    ...
    async def preview_remote(
        ...
        try:
            ...
            if decision.timing == "immediate":
                ...
                estimate = await self.gateway.preview_immediate_plan_change(context)
                safe = (
            ...
            final = await self._store_preview(
                str(row["id"]), lease_token, decision, context, estimate
            )
    ...
    async def confirm(
        ...
        lease_token = uuid.uuid4()
        leased = await self._acquire_lease(str(row["id"]), lease_token, "previewed")
        if leased is None:
    ...
    async def _reserve(
        ...
        async with self.pool.acquire() as conn, conn.transaction():
            ...
            if account["subscription_status"] != "active" or not account[
            ...
            current_interval = self._interval(str(account["plan_interval"]))
            current = self.catalog.require(current_key)
            decision = decide_transition(current, current_interval, target, target_interval)
    ...
    async def _revalidate_before_remote(
        ...
        if context.current_lookup_key not in {expected_lookup, target_lookup}:
    ...
    async def _release_after_error(
        ...
        async with self.pool.acquire() as conn:
            await conn.execute(
                """update billing_plan_changes set lease_token=null,
    ...
    async def _assert_account_snapshot(self, reserved: Mapping[str, Any]) -> None:
        ...
        async with self.pool.acquire() as conn, conn.transaction():
            ...
            if change is not None and change["status"] == "completed":
                return
            if self._account_matches_snapshot(account, reserved):
                return
            mismatch = True
            await conn.execute(
    ...
    @staticmethod
    def _account_matches_snapshot(
        account: Mapping[str, Any], reserved: Mapping[str, Any]
    ) -> bool:
        return bool(
