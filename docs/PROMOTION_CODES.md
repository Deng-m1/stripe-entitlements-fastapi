...
- Hosted Checkout never receives `allow_promotion_codes` on this branch. Enabling that
  Session parameter before Phase-2 attribution ships would charge discounted amounts
  without granting entitlements; see Phase 1.
...
| `src/stripe_entitlements/config.py`, health surface | Only after Phase-2 attribution exists: expose an explicit, reviewed enablement surface (not a silent Session-only toggle); document charged-but-not-entitled risk until acceptance ships |
...
- **R1 — Early Session enablement.** Passing `allow_promotion_codes` to Checkout on a production
  path before Phase 2 ships creates charged-but-not-entitled customers.
